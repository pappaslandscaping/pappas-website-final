...
function catalogAnswer(question) {
  ...
  if (/\b(snow|plow|plowing)\b/.test(q)) {
    return "We offer seasonal snow-removal contracts, not per-push plowing. Driveway service is scheduled after 2 inches or more of new snowfall. Timing depends on the storm and our route, so we can't guarantee a specific arrival time. Winter route openings are limited; request a quote so our team can confirm availability and any walkway or deicing options.";
  }
